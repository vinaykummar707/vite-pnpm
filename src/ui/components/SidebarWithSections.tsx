  ...
  return (
    <div
      className={SubframeUtils.twClassNames(
        ...
        {
          "items-center justify-center bg-brand-200 hover:bg-brand-50 active:bg-brand-100":
            selected,
        },
        className
      )}
...
const NavSection = React.forwardRef<HTMLDivElement, NavSectionProps>(
  function NavSection(
    ...
    return (
      <div
        className={SubframeUtils.twClassNames(
          "flex w-full flex-col items-start gap-1 pt-2",
          className
        )}
        ref={ref}
        {...otherProps}
...
>(function SidebarWithSectionsRoot(
  ...
  return (
    ...
    >
      {header ? (
        <div className="flex w-full items-start gap-1 px-6 py-6">{header}</div>
      ) : null}
      {children ? (
        <div className="flex w-full grow shrink-0 basis-0 flex-col items-start px-4 pt-1 pb-4 overflow-auto">
          {children}
        </div>
      ) : null}
      {footer ? (
        <div className="flex w-full items-center gap-4 border-t border-solid border-neutral-border px-6 py-4">
          {footer}
        </div>
      ) : null}
    </nav>
